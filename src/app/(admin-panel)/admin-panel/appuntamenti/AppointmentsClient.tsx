...
export function AppointmentsClient() {
  ...
  const [dateTo, setDateTo] = useState(() => formatDateStr(addDays(new Date(), 7)))
  ...
  const getAppointmentsForDay = (date: Date) => {
    ...
    return filteredAppointments.filter((apt) => new Date(apt.date).toISOString().split('T')[0] === ds)
  }
  ...
  return (
    <div className="space-y-4 admin-fade-in">
      ...
      <div className="admin-card p-2.5 flex flex-wrap items-center gap-2">
        <button
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0">
                            {apt.clientPhone && (
                              <>
                                ...
                                <a
                                  href={`https://wa.me/${apt.clientPhone.replace(/\s+/g, '').replace(/^\+/, '')}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="p-1.5 rounded-md hover:bg-green-500/10 text-green-500/60 hover:text-green-400 transition-colors"
                                  title="WhatsApp"
                                >
