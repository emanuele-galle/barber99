...
export function DeleteServiceButton({ serviceId, serviceName }: DeleteServiceButtonProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast('success', `Servizio "${serviceName}" eliminato`, 'Eliminato')
      router.refresh()
    } catch (error) {
      console.error('Delete error:', error)
      showToast('error', 'Errore durante eliminazione del servizio', 'Errore')
    } finally {
      setLoading(false)
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="admin-modal p-6 relative admin-fade-in">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">Elimina servizio</h3>
            <p className="text-[rgba(255,255,255,0.6)] mb-6">
              Sei sicuro di voler eliminare <span className="text-white font-medium">{serviceName}</span>? Questa azione non può essere annullata.
            </p>
